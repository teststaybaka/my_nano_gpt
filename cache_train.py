"""Train script for the next-token-aligned cache-carry models:
  rope_sliding      → rope_sliding_cache_model  (chunk T = block_size)
  rope_stair        → rope_stair_model          (chunk T = block_size/2, stair invariant)
  multi_scale_stair → multi_scale_stair_model   (chunk T = block_size/2, stair invariant)

Same recipe as sliding_cache_train.py (BPTT over context_length with cache carry,
2**19 tokens per optimizer step, identical LR schedule), minus the pair-sum seed
token: these models predict idx[i+1] from idx[:i+1] directly, so chunks start at
position 0 and targets are the data loader's standard next-token y.

Usage:
  python cache_train.py --model {rope_sliding|rope_stair|multi_scale_stair}
                        [--context-length 4096]
"""
import os
import time
    ...
    for _ in range(val_loss_steps):
      ...
      for sub in range(sub_chunks_per_batch):
        ...
        with torch.autocast(device_type=device, dtype=torch.bfloat16):
          _, loss, caches = model(idx, targets, caches)
...
# Hyperparameters (matching sliding_cache_train.py)
...
for i in range(max_iters):
  ...
  for micro_step in range(grad_accum_steps):
    ...
    # BPTT across sub_chunks_per_batch chunks: the cache carries forward with
    # gradient attached, so loss.backward() at the end reaches all of them.
    for sub in range(sub_chunks_per_batch):
      start = sub * T
      idx     = x[:, start : start + T]
      targets = y[:, start : start + T]
