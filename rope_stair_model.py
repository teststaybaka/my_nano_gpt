...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, caches=None):
        ...
        targets: (B, T_new) — standard next-token targets: targets[i] is the token after
                 idx[i]. None at eval skips loss.
        ...
        x = self.wte(idx) # (B, T_new, n_embd) — position enters via RoPE in attn

        # Forward through layers, collecting each layer's K[l], V[l].
        computed_kv = []
        ...
        # Build new caches — same pointer shuffle as stair_model.
        new_caches = []
        n_layers = len(self.h)
