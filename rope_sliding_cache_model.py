...
class GPT(nn.Module):
  ...
  def forward(self, idx, targets=None, kv_caches=None):
    ...
    targets: (B, T_new) — standard next-token targets: targets[i] is the token after idx[i].
             Pass None at eval to skip the loss.
    ...
    x = self.transformer['wte'](idx) # (B, T_new, n_embd) — position enters via RoPE in attn
